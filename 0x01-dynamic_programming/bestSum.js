...
/**
 * /**bestSum has exponential time complexity and quadratic space complexity
 * 
 ...
 * 
 * every recursive call will have its own `shortestCombination` variable; worst-case scenario, length will be m 
 * m variables with values of length m, at worst, equals (m*m) or (m^2) max space required 
 * 
...
const bestSumMemo = (targetSum, numbers, memo={}) => {
    ...
    for(let i of numbers){
        ...
        if(remainderCombination!==null){
            const combination = [...remainderCombination, i]
            if(shortestCombination===null || combination.length<shortestCombination.length){
                shortestCombination = combination
            }
        }
...
arr.forEach(obj => {
    console.log(`input: (${obj.targetSum}, [${obj.numbers}]): return: ${bestSumMemo(obj.targetSum, obj.numbers)}`);
})

/**
 * bestSumMemo has quadratic time and space complexity
 * 
 * #recursive calls are the same as bestSum -> n*m.
 ...
 * 
 * recall, for bestSum: time complexity is O((n^m) * m) and space complexity is O(m^2)
 * trade-offs
